...
```{r Initial setup and preprocessing, message=F, warning=F, echo=TRUE}
#### Initial setup ####
...
```{r task 1 histogram, message=F, warning=F, echo=TRUE}
ggplot(task1_df, aes(x=steps)) + geom_histogram()
```

### Mean and median number of steps taken each day
#### The mean is `r round(mean(task1_df$steps, na.rm = TRUE), 1)`, and the median is `r round(median(task1_df$steps, na.rm = TRUE), 1)`


## What is the average daily activity pattern?
### Time series plot of the average number of steps taken
```{r task 2 Time series, message=F, warning=F, echo=TRUE}
ggplot(data = task2_df, aes(x = interval, y = steps))+
    geom_line()
```

```{r task 2 Max number of steps, echo=TRUE}
...
#### The 5-minute interval that, on average, contains the maximum number of steps is `r max_steps_interval`
...
#### Total number of missing values in the dataset is `r sum(is.na(df))`


### Histogram of the total number of steps taken each day after missing values are imputed
```{r task 3 histogram, message=F, warning=F, echo=TRUE}
ggplot(task3_df, aes(x=steps)) + geom_histogram()
```


#### The mean is `r round(mean(task3_df$steps, na.rm = TRUE), 1)`, and the median is `r round(median(task3_df$steps, na.rm = TRUE), 1)`
...
```{r task 4 panel plot, message=F, warning=F, echo=TRUE}
