...
```{r task1_histogram, message=F, warning=F, echo=TRUE}
...
```{r task2_Time series, message=F, warning=F, echo=TRUE}
ggplot(data = task2_df, aes(x = interval, y = steps))+
    geom_line()
```

```{r task2_MaxNsteps, echo=TRUE}
...
```{r task3_histogram, message=F, warning=F, echo=TRUE}
...
```{r task4_panelplot, message=F, warning=F, echo=TRUE}
